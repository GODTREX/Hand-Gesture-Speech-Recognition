...
def change_volume(action):
    try:
        ...
        if action == "increase":
            ...
            volume.SetMasterVolumeLevelScalar(0.0, None)  # Set volume to zero (not mute)
...
def perform_action(finger_count):
    ...
    try:
        ...
        if finger_count == 0:
            change_volume("min")  # Set volume to zero
...
def perform_voice_action(command):
    ...
    if "louder" in command:
        ...
        change_volume("min")  # Set volume to zero (not mute)
        print("Volume set to zero.")
